...
public class AzureADServices implements LDAPServices {
    ...
    public void testLdapConnection(LDAPServerConfiguration ldapServerConfiguration, String user, String password)
        ...
        try {
            if (getAccessToken(ldapServerConfiguration) == null) {
    ...
    private String getAccessToken(LDAPServerConfiguration ldapServerConfiguration) throws Throwable {
        ...
        try {
            ...
            Future<AuthenticationResult> authenticationResultFuture = authenticationContext.acquireToken(ldapServerConfiguration.getResource(),
                    new ClientCredential(ldapServerConfiguration.getClientId(), ldapServerConfiguration.getClientSecret()),
                    null);
	...
	public List<String> getTestSynchronisationUsersNames(final LDAPServerConfiguration ldapServerConfiguration,
        ...
        String accessToken;
        try {
            accessToken  = getAccessToken(ldapServerConfiguration);
        } catch (Throwable t) {
                            ...
                            for (int j = 0, groupMembersJsonArrayLength = groupMembersJsonArray.length(); j < groupMembersJsonArrayLength; j++){
                                ...
                                if (groupMemberUrl.endsWith("Microsoft.DirectoryServices.User")) {
                                    ...
                                    if (response.getStatus() == HttpURLConnection.HTTP_OK) {
                                        ...
                                        ldapUser.setLastLogon(new DateTime(groupUserJsonObject.optString("refreshTokensValidFromDateTime")).toDate());
                                    }
                                }
                            }
                        } else {
                ...
                try {
                    ...
                    if (ldapUserSyncConfiguration.isUserAccepted(userDisplayName)) {
                        ...
                        if (response.getStatus() == HttpURLConnection.HTTP_OK) {
                            ...
                            for (int j = 0, userGroupsJsonArrayLength = userGroupJsonArray.length(); j < userGroupsJsonArrayLength; j++){
                                ...
                                if (userGroupUrl.endsWith("Microsoft.DirectoryServices.Group")) {
                                    ...
                                    if (response.getStatus() == HttpURLConnection.HTTP_OK) {
                                        ...
                                        LDAPGroup ldapGroup = new LDAPGroup(userGroupJsonObject.optString("displayName"), userGroupJsonObject.optString("objectId"));
