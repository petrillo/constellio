...
public class SolrServers {
	...
	public synchronized BigVaultServer getSolrServer(String core) {
		...
		if (server == null) {
			SolrBigVaultServer solrServer = new SolrBigVaultServer(core, bigVaultLogger, solrServerFactory,
					extensions.getSystemWideExtensions());
			server = new IgnitedBigVaultServer(solrServer);
